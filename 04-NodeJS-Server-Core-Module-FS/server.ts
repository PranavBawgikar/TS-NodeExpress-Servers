...
const server:Server = http.createServer((request:IncomingMessage, response:ServerResponse) => { //Creating a new HTTP Server instance
    ...
    /*Similarly, you can fetch data from a JSON file as well
    fs.readFile(path.join(__dirname, 'data', 'weather.json'), 'utf-8', (error, result) => { //Allocating the path to the file dynamically by using the path module
        if(error){
            console.log(error);
        }
